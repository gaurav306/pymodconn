...
class Build_utils():
	...
	def __init__(self,configs,current_dt):
		...
		self.save_training_history_file     = os.path.join(self.save_results_dir, '%s.csv' % (current_dt))
		self.save_training_history          = os.path.join(self.save_results_dir, '%s_history.png' % (current_dt))
		self.save_hf5_name                  = os.path.join(self.save_models_dir, '%s.h5' % (current_dt))
		self.save_modelimage_name           = os.path.join(self.save_models_dir, '%s_modelimage.png' % (current_dt))
		self.save_modelsummary_name         = os.path.join(self.save_models_dir, '%s_modelsummary.txt' % (current_dt))
	...
	def postbuild_model(self, model) -> Tuple[float, int]:
		...
		self.GET_MODEL_SIZE_GB = get_model_memory_usage(self.batch_size, self.model)
		...
		if self.if_save_model_image:
			plot_model(self.model, to_file=self.save_modelimage_name, show_shapes=True, show_layer_names=True, dpi=600, expand_nested=True)     
		print("[ModelClass_tfp] Building postmodel DONE!!") 

		return self.GET_MODEL_SIZE_GB, self.trainable_count
...
class rnn_unit():
	def __init__(self, configs, rnn_location, num):
		...
		self.all_layers_neurons = configs['model']['all_layers_neurons']
		self.all_layers_neurons_rnn = int(self.all_layers_neurons/self.input_enc_rnn_depth)
	...
	def single_rnn_layer(self, x_input, init_states, mid_layer=False, layername_prefix=None):
		...
		if self.input_enc_rnn_bi:
			self.layername = layername_prefix + self.rnn_location + '_' + str(self.num) + '_bi' + self.rnn_type
		else:
			self.layername = layername_prefix + self.rnn_location + '_' + str(self.num) + '_' + self.rnn_type
	...
	def __call__(self, input_to_layers, init_states=None):
		if self.input_enc_rnn_depth == 1:
			...
			x = self.single_rnn_layer(x_input=input_to_layers, init_states=init_states, mid_layer=True , layername_prefix='First_')   #change
			x = glu_with_addnorm(self.configs, input_to_layers, x)
			for i in range(0, self.input_enc_rnn_depth-2):
				x = self.single_rnn_layer(x_input=x, init_states=init_states, mid_layer=True, layername_prefix='Mid_%s_' % (i+1))
				x = glu_with_addnorm(self.configs, input_to_layers, x)
